...
def get_data(term, num_features=8555, cols=None, pca_dimen=None):
    ...
    neg_file_name = exp_input_file_path + term + '_neg_0.txt'
...
def logistic_regresssion_L1(term, x_tr, x_te, y_tr, y_te, gene_ids_test, idx_te, server,
                            tissues=None, rand_permute=False, pca_dimen=None):
    ...
    # Save results
    if tissues:
        directory_path = 'pca_results_1_tissue_server_' + str(server) + '/' + term + '_server_' + str(server)
        out_fname = directory_path + '/logreg_' + tissues[0] + '.txt'
    else:
        directory_path = 'pca_results_all_tissues_server_' + str(server)
...
def copy_zipped_results_to_S3(server, tissue_specific=False):
    ...
    #          's3://stanfordgtex/GO_Prediction/PCA_GO_Prediction_Results/results_server_' + str(server) + '.tar.gz')
    os.system('aws s3 cp ' + tar_name + '.tar.gz'
              ' s3://stanfordgtex/GO_Prediction/PCA_GO_Prediction_Results/' + tar_name)
...
def run_prediction(GO_term, server_no, tissues=None, cols=None, pca_dimen=None):
...
def predict_GO_to_genes(n_servers, server_no, tissue_specific=False, tissue_fpath=None, pca_dimen=None):
    ...
    for (idx, GO_term) in enumerate(GO_terms):
        ...
        if tissue_specific:
            for tissue in tissues:
                run_prediction(GO_term, server_no, tissues=[tissue], cols=tissues_to_cols[tissue], pca_dimen=pca_dimen)
        else:
            run_prediction(GO_term, server_no, pca_dimen=pca_dimen)
...
if __name__ == "__main__":
    ...
    #predict_GO_to_genes(num_servers, server_number, pca_dimen=5)

    # Make predictions using expression in individual tissues
    predict_GO_to_genes(num_servers, server_number, tissue_specific=True, tissue_fpath='tissues.txt', pca_dimen=5)
